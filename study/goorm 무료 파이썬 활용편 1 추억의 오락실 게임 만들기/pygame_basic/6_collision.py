...
character_size = character.get_rect().size  # 이미지 크기를 리턴
...
# 적 enemy 캐릭터
enemy = pygame.image.load("raw/enemy.png")
enemy_size = enemy.get_rect().size  # 이미지 크기를 리턴
enemy_width = enemy_size[0]  # 캐릭터의 가로 크기
enemy_height = enemy_size[1]  # 캐릭터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)  # 화면 가로의 절반 크기에 해당하는 곳에 위치 (가로 위치)
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)  # 화면 세로의 가장 아래에 해당하는 곳에 위치 (세로 위치)
...
while running:
    ...
    # 캐릭터가 100 만큼 이동을 해야함
    # 10 FPS: 1초 동안에 10번 동작 -> 1번에 몇 만큼 이동? 10만큼 이동
    ...
    for event in pygame.event.get():  # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.QUIT or (
                event.type == pygame.KEYDOWN and event.key == pygame.K_q):  # 창이 닫히는 이벤트가 발생하였는가?
