...
balls = [{
    ...
    "to_y": -6,  # y축 이동방향,
    "init_spd_y": ball_speed_y[0]  # y 최소속도
}]
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_SPACE:  # 무기 발사 위치 정의
    ...
    for ball_idx, ball_val in enumerate(balls):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "to_x": -3,  # x축 이동방향, -3이면 왼쪽으로 3이면 오른쪽으로
                        "to_y": -6,  # y축 이동방향,
                        "init_spd_y": ball_speed_y[ball_img_idx + 1]  # y 최소속도
                    })
        ...
        break  # 안쪽 for 문에서 break 를 만느면 여기로 진입 가능, 2중 for 문을 한번에 탈출
    ...
    timer = game_font.render(f"Time: {int(total_time - elapsed_time)}", True,
                             (255, 255, 255))  # 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 표시
    screen.blit(timer, (10, 10))  # 출력할 글자, True, 글자 색상
